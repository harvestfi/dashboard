...
async function isContract(address, provider) {
  ...
  console.log(
    `memoizing ${address} is ${CONTRACT_MEMO[address]} with code length ${
      code.length / 2 - 1
    }`,
  )
...
async function processPool(pool, currentBlock) {
  ...
  const processPoolAwaitRes = await batchMap(events, 5, async (e) => {
    const processEventRes = await processEvent(pool, e)
    return processEventRes
  })
  return processPoolAwaitRes
}
...
async function retrieveAll() {
  ...
  try {
    await batchMap(man.pools, 2, async (pool) => {
      const processPoolRes = await processPool(pool, block)
      return processPoolRes
    })
  } catch (e) {
    console.error(e)
  } finally {
